...
#### Visualize `car_simulation.py` in Foxglove

`car_simulation.py` publishes ROS1 topics (pose, TF, markers, route) which can be viewed in Foxglove via rosbridge.
...
Run the simulation + rosbridge and forward the websocket port:
```bash
docker run --rm -it \
  -p 9090:9090 \
  -v "$PWD/car_simulation.py":/home/developer/workspace/src/lanelet2/car_simulation.py:ro \
  lanelet2:latest /bin/bash -lc \
  'source /opt/ros/noetic/setup.bash && \
   source /home/developer/workspace/devel/setup.bash || true && \
   roscore & \
   roslaunch rosbridge_server rosbridge_websocket.launch address:=0.0.0.0 port:=9090 & \
   python3 /home/developer/workspace/src/lanelet2/car_simulation.py'
```

To force a specific route, set parameters before starting the script:
```bash
rosparam set /lanelet2_car_simulation/start_lanelet_id 42968
rosparam set /lanelet2_car_simulation/end_lanelet_id  38564
```

Lanelet ID labels are enabled by default (only within a bbox around the route). To disable:
```bash
rosparam set /lanelet2_car_simulation/show_lanelet_ids false
```

In Foxglove Studio, add a "Rosbridge" connection to `ws://localhost:9090`.
...
python3 lanelet2_python/scripts/print_ids.py lanelet2_maps/res/boston-seaport.osm
